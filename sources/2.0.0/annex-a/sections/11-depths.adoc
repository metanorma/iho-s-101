...
stem:[bb "depth range minimum value" = 5] and
stem:[bb "depth range maximum value" = 20].

NOTE: In <<fig_11-3>>, if the optional *Depth Area* features are
encoded, the depth area (_abcda_) will be split into two separate
*Depth Area* features (_abgea_) and (_jhcdj_), both having
stem:[bb "depth range minimum value" = 5] and
stem:[bb "depth range maximum value" = 20].
...
bathymetry, it must be done by populating the attribute
stem:[bb "category of temporal variation" = 2] (likely to change and
...
data, as described above, however with
stem:[bb "category of temporal variation" = 2]
...
stem:[bb "data assessment" = 1] (assessed), *features detected*
(*least depth of detected features measured* and
*significant features detected*) = _False_ and
*full seafloor coverage achieved* = _False_; and (if
