...
using a *Building* or *Landmark* feature, with attribute stem:[bb
"function" = 11] (pilot office) or _12_ (pilot lookout).
...
a *Special Purpose/General Beacon* feature, with attribute stem:[bb
"category of special purpose mark" = 44] (refuge beacon), not by
using *Rescue Station*.
* Each VHF-channel should be indicated, using the attribute
*communication channel* (see <<sec_27.77>>). Alternately, if the same
VHF-channel(s) apply to multiple features in the dataset, this should
be indicated through an associated instance of the information type
...
*Harbour Facility* with attribute stem:[bb "category of harbour
facility" = 10] (container terminal).
