...
| (S) EN
| 0,1 footnote:crossnote[Complex attribute *feature name*, sub-attribute *name usage* is mandatory if the name is intended to be displayed when display of names is enabled by the Mariner. See <<sec_2.5.8>>.]
...
* Where it is required to encode a mangrove area or tree located in the intertidal area, this should be done using the feature *Obstruction* (see clauses 13.6 and 13.6.1.1), with attribute *category of obstruction* = _23_ (mangrove). Where it is required to encode the generalised seaward edge only of a mangrove area to represent the "apparent" coastline, this must be done using the feature *Coastline* (see <<sec_5.3>>).
