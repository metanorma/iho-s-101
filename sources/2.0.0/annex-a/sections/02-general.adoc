...
in the S-101 Feature Catalogue (see <<S101,clause=4.3>>).
It provides the encoding rules and guidance
...
real number is constrained to a precision of 0.01; etc)
(<<IHO_S_100,part=2a,clause=2a-4.2.10>>). For the attribute descriptions included in
...
of the basic format as defined in <<ISO_8601_2004>>. Complete representation
...
of such seamless presentations (see <<sec_3.5>> and
<<S101,clause=4.6>> and <<S101,clause=4.7>>).
...
Encoded date ranges are inclusive, see <<IHO_S_100,part=3,clause=3-8.3>>.
For example:
...
See also <<S101,clause=4.5>> in addition to the sub-clauses
...
| _&#x157F;&#x146D;&#x1575;&#x1456;&#x14D7;&#x14C3;_       | _iku_ | _-_ .<| Alternate (Spanish)   .<| Baffin Island
...
Mariner certain edges of features should be masked (see
<<S101,clause=4.8.3>>). For example, the boundaries
...
to assist with portrayal of ENC data in ECDIS (see <<sec_30>>). These
...
is bound. Such bindings are defined in <<S101>>.
