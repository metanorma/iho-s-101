...
<<IHO_S_100,part=17,figure=17-1>>, <<IHO_S_100,part=17,figure=17-6>>
and <<IHO_S_100,part=17,figure=17-7>> outline the overall concept
of an S-100 Exchange Set for the interchange of geospatial data and
its relevant metadata. <<IHO_S_100,part=17,figure=17-1>> depicts the realization of the
<<ISO_19115_1>> and <<ISO_TS_19115_3>> classes which form the foundation of the Exchange
Set. The overall structure of S-101 metadata for Exchange Sets is
the same as S-100 metadata. <<IHO_S_100,part=17,figure=17-6>> depicts the structure of the
Exchange Set Catalogue and the structure of the Exchange Set as included
in <<IHO_S_100,part=17,figure=17-1>> is also described in <<sec_11.2>> above.
More detailed information about the various classes is shown in
<<IHO_S_100,part=17,figure=17-7>> and a textual description in the Tables is included at
<<sec_12.1.1;to!sec_12.1.4>> below.
...
given in <<IHO_S_100,part=17>>, and a comment noting the restricted multiplicity
...
File Discovery Metadata and Catalogue Discovery Metadata from
<<IHO_S_100,part=17>> with additional S-101-specific restrictions.
...
| Content defined in <<IHO_S_100,part=15>>. All certificates used, except the SA root certificate (installed separately by the implementing system) shall be included
...
| fileName | Dataset file name | 1 | URI | See <<IHO_S_100,part=1,clause=1-4.6>>
...
| compressionFlag | Indicates if the resource is compressed | 1 | Boolean | _True_ indicates a compressed dataset resource

__False__ indicates an uncompressed dataset resource
| dataProtection | Indicates if the data is encrypted | 1 | Boolean | _True_ indicates an encrypted dataset resource

__False__ indicates an unencrypted dataset resource
| protectionScheme
...
| digitalSignatureValue | Value derived from the digital signature | 1..* | S100_SE_DigitalSignature (see <<IHO_S_100,part=15>>) | The value resulting from application of digitalSignatureReference.Implemented as the digital signature format specified in <<IHO_S_100,part=15>>
| copyright | Indicates if the dataset is copyrighted | 1 | Boolean | _True_ indicates the resource is copyrighted

__False__ indicates the resource is not copyrighted
| classification
...
| notForNavigation | Indicates the dataset is not intended to be used for navigation | 1 | Boolean | _True_ indicates the dataset is not intended to be used for navigation

__False__ indicates the dataset is intended to be used for navigation
| specificUsage
...
| See <<IHO_S_100,part=17,table=17-3>>
...
| 0..*
| PT_Locale |
| metadataPointOfContact | Point of contact for metadata | 0..1 | CI_Responsibility>CI_Individual or CI_Responsibility>CI_Organisation | Only if metadataPointOfContact is different to producingAgency
...
| resourceMaintenance | Information about the frequency of resource updates, and the scope of those updates | 0..1 | MD_MaintenanceInformation | S-100 restricts the multiplicity to 0..1 and adds specific restrictions on the ISO 19115 structure and content. See clause MD_MaintenanceInformation later in this PartFormat: PnYnMnDTnHnMnS (XML built-in type for ISO 8601 _duration_). See <<IHO_S_100,part=17,clause=17-4.9>>
...
| Enumeration | S100_EncodingFormat | The encoding format | - | Values listed in <<IHO_S_100,part=17>> but not mentioned in this table are not allowed
| Value
| ISO/IEC 8211
| The ISO 8211 data format as defined in <<IHO_S_100,part=10a>>
| *1*
|
...
| fileName | Name of the support file | 1 | URI | See <<IHO_S_100,part=1,clause=1-4.6>>
...
| dataType | The format of the support file | 1 | S100_SupportFileFormat | Constrained to TXT and TIF - see <<sec_11.4.1>>
| comment
| Optional comment
| 0..1
| CharacterString
...
| compressionFlag | Indicates if the resource is compressed | 1 | Boolean | _True_ indicates a compressed resource

__False__ indicates an uncompressed resource
| digitalSignatureReference
| Specifies the algorithm used to compute digitalSignatureValue
| 1
| S100_SE_DigitalSignatureReference (see <<IHO_S_100,part=15>>)
...
| digitalSignatureValue | Value derived from the digital signature | 1..* | S100_SE_DigitalSignature (see <<IHO_S_100,part=15>>)
...
| supportedResource | Identifier of the resource supported by this support file | 0..* | CharacterString | Conventions for identifiers are detailed in <<IHO_S_100,part=15>>. S-100 allows file URI, digital signature or cryptographic hash checksums to be used
...
| Enumeration | S100_SupportFileFormat | The format used for the support file | - | Values listed in <<IHO_S_100,part=17>> but not mentioned in this table are not allowed
...
| fileName | The name for the Catalogue | 1 | URI | See <<IHO_S_100,part=1,clause=1-4.6>>
...
| digitalSignatureReference | Specifies the algorithm used to compute digitalSignatureValue | 1 | S100_SE_DigitalSignatureReference (see <<IHO_S_100,part=15>>) | Reference to the appropriate digital signature algorithm
| digitalSignatureValue | Value derived from the digital signature | 1..* | S100_SE_DigitalSignature (see <<IHO_S_100,part=15>>) | The value resulting from application of digitalSignatureReferenceImplemented as the digital signature format specified in Part 15
...
The class PT_Locale is defined in <<ISO_19115_1>>. LanguageCode, CountryCode, and MD_CharacterSetCode are ISO codelists which are defined in a resource file in the S-100 Edition 5.0.0 Schemas distribution.
